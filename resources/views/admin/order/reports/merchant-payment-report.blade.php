                            ...
                            <div class="row">
                                <div class="col-lg-12">
                                    <div class="mr-table allproduct">
                                        ...
                                        <input type="text" class="form-control-sm" name="from_date" id="from" placeholder="{{ __('Select a date') }}" value="{{ $from }}" autocomplete="off" style="width: 100px;">
                                                ...
                                                <table id="geniustable" class="table table-hover dt-responsive" cellspacing="0" width="100%">
                                                    <thead>
                                                        <tr>
                                                            ...
                                                            <th>16. Total of Bill</th>
                                                            <th>17. Other cards</th>
...
<script type="text/javascript">
        ...
        var fromformat = from.getFullYear() + '-' + (from.getMonth()+1) + '-' + from.getDate();
        var to = new Date($("#to").val());
        var toformat = to.getFullYear() + '-' + (to.getMonth()+1) + '-' + to.getDate();
        // var status = $('#status').val();
        var status = 'completed';
        var iscollected = $('#is-collected').val();
        table.destroy();
        var url = mainurl+'/admin/orders/datatablesv2/report/merchantp/'+status+'/'+iscollected+'/'+fromformat+'/'+toformat;
        // var url = mainurl+'/admin/orders/datatablesv2/report/merchantp';
        // console.log(url);
        table = $('#geniustable').DataTable({
            ...
            columns: [
                ...
                { data: '_16_Total_of_bill', name: '_16_Total_of_bill' },
                { data: '_17_Other_cards', name: '_17_Other_cards' },
    ...
    $("#add-find" ).on('click' , function(e){
        ...
        $.ajax({
            type:"GET",
            url:mainurl+'/admin/orders/datatablesv2/report/merchantp/'+status+'/'+iscollected+'/'+fromformat+'/'+toformat,
            success: function(data){
                // console.log(url);
                $("#export-excel" ).removeClass("disable-click");
                table.ajax.url( mainurl+'/admin/orders/datatablesv2/report/merchantp/'+status+'/'+iscollected+'/'+fromformat+'/'+toformat ).load();
                // table.ajax.url( mainurl+'/admin/orders/datatablesv2/report/merchantp/'+status+'/'+iscollected+'/'+sf+'/'+st ).load();
    ...
    $("#export-excel" ).on('click' , function(e){
        ...
        var status = 'completed';
        var iscollected = $('#is-collected').val();
        var url = mainurl+'/admin/orders/export/report/merchantp/'+status+'/'+iscollected+'/'+sf+'/'+st;
