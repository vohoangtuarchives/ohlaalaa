									...
									<tbody>
										<tr>
											...
											<td class="product-image-title c{{$product['item']['id']}}">
                                                    ...
                                                    @if (isset($product['item']['thumbnail']))
                                                        @if (strncmp($product['item']['thumbnail'], "Data/", 5) === 0)
                                                        <img class="img-fluid" src="{{ asset('assets/images/'.$product['item']['thumbnail'])}}" alt="Compare product['item']">
                                                        @else
                                                        <img class="img-fluid" src="{{ asset('assets/images/thumbnails/'.$product['item']['thumbnail']) }}" alt="Compare product['item']">
                                                        @endif

                                                    @else
													<img class="img-fluid" src="{{ asset('assets/images/noimage.png') }}" alt="Compare product['item']">
                                                    @endif
													{{-- <img class="img-fluid" src="{{ $product['item']['thumbnail'] ? asset('assets/images/thumbnails/'.$product['item']['thumbnail']):asset('assets/images/noimage.png') }}" alt="Compare product['item']"> --}}
										...
										<tr>
											...
											@foreach($products as $product)
											<td class="pro-price c{{$product['item']['id']}}">{{ App\Models\Product::find($product['item']['id'])->showPrice() }}<br>
                                                @if ($product['item']->price_shopping_point > 0)
                                                SP {{ number_format($product['item']->price_shopping_point) }} <small>{{ $product['item']->percent_shopping_point }}%</small>
                                                @endif
                                            </td>
											@endforeach
										</tr>
