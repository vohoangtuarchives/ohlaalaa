									...
									<div class="col-lg-4 col-md-4 col-6 remove-padding">
										...
										<a href="{{ route('front.product', $prod->slug) }}" class="item">
											<div class="item-img">
												@if(!empty($prod->features))
													<div class="sell-area">
													  @foreach($prod->features as $key => $data1)
														<span class="sale" style="background-color:{{ isset($prod->colors[$key]) ? $prod->colors[$key] : '' }}">{{ $prod->features[$key] }}</span>
														@endforeach
													</div>
												@endif
												...
												<img class="img-fluid" src="{{ $prod->show_thumbnail() }}" alt="" style="height: 283px">
											</div>
											<div class="info">
												<div class="stars">
                            <div class="ratings">
