            ...
            <div class="body-area">
              <form id="catalogForm" action="{{ route('front.category', [Request::route('category'), Request::route('subcategory'), Request::route('childcategory')]) }}" method="GET">
                ...
                <ul class="filter-list">
                  ...
                  <li>
                    <div class="content">
                        <?php   $sel = 'style="color: #f20f2f"'; ?>
                        @if (Request::route('category') == $element->slug)
                            <a <?php echo $sel;?> href="{{route('front.category', $element->slug)}}{{!empty(request()->input('search')) ? '?search='.request()->input('search') : ''}}" class="category-link"> <i class="fas fa-angle-double-right"></i> {{$element->name}}</a>
                        @else
                        ...
                        @if(!empty($cat) && $cat->id == $element->id && !empty($cat->subs))
                            ...
                            <div class="sub-content open">
                                @if (Request::route('subcategory') == $subelement->slug)
                                    <a <?php echo $sel;?> href="{{route('front.category', [$cat->slug, $subelement->slug])}}{{!empty(request()->input('search')) ? '?search='.request()->input('search') : ''}}" class="subcategory-link"><i class="fas fa-angle-right"></i>{{$subelement->name}}</a>
                                @else
                              ...
                              @if(!empty($subcat) && $subcat->id == $subelement->id && !empty($subcat->childs))
                                ...
                                <div class="child-content open">
                                    @if (Request::route('childcategory') == $childcat->slug)
                                        <a <?php echo $sel;?> href="{{route('front.category', [$cat->slug, $subcat->slug, $childcat->slug])}}{{!empty(request()->input('search')) ? '?search='.request()->input('search') : ''}}" class="subcategory-link"><i class="fas fa-caret-right"></i> {{$childcat->name}}</a>
                                    @else
