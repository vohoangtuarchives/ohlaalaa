                            ...
                            <div class="row">
                                <div class="col-lg-12">
                                        ...
                                        <ul class="links">
                                            ...
                                            <li>
                                                <a href="{{ route('admin-user-report-sale-report-datatables',[\Carbon\Carbon::now()->format('Y-m-d'),\Carbon\Carbon::now()->format('Y-m-d'), 1, 1])}}">{{ __('Sale Report Summaries Top 10 Spending Order') }}</a>
                                            </li>
                                        </ul>
                                </div>
                            </div>
...
<script type="text/javascript">
    ...
    var table = $('#geniustable').DataTable({
        ...
        serverSide: true,
        pageLength: 10,
        ajax: '{{ route('admin-user-report-sale-report-datatables',[\Carbon\Carbon::now()->format('Y-m-d'),\Carbon\Carbon::now()->format('Y-m-d'), 1, 1])}}',
        columns: [
    ...
    $("#add-find" ).on('click' , function(e){
        ...
        var url = mainurl+'/admin/users/reports/salereport/datatables/'+sf+'/'+st+'/1'+'/1';
        console.log('url',url);
    ...
    $("#export-excel" ).on('click' , function(e){
        ...
        var st = get_date_string($('#to').val());
        // var status = $('#status').val();
        var url = mainurl+'/admin/users/reports/salereport/export/'+sf+'/'+st+'/1'+'/1';
