...
// **************************************  AJAX REQUESTS SECTION *****************************************
  ...
  // Droplinks Start
      ...
      $(document).on('change','.rank-droplinks',function () {
        var link = $(this).val();
        var data = $(this).find(':selected').attr('data-val');
        var cls = $(this).find(':selected').attr('data-class');
        $(this).next(".nice-select.process.select.rank-droplinks").removeClass("drop-regular");
        $(this).next(".nice-select.process.select.rank-droplinks").removeClass("drop-premium");
        $(this).next(".nice-select.process.select.rank-droplinks").removeClass("drop-gold");
        $(this).next(".nice-select.process.select.rank-droplinks").removeClass("drop-platinum");
        $(this).next(".nice-select.process.select.rank-droplinks").addClass(cls);

        $('#confirm-update').modal('show');
        $('#confirm-update').find('.btn-ok').attr('href', $(this).val());
...
// INVOICE UPDATE OPERATION

$(document).on('click','.invoice-update',function(){
if(admin_loader == 1)
  {
  $('.submit-loader').show();
}
  $('#modal1').find('.modal-title').html('INVOICE UPDATE INFO');
  $('#modal1 .modal-content .modal-body').html('').load($(this).attr('data-href'),function(response, status, xhr){
      if(status == "success")
      {
        if(admin_loader == 1)
...
// COLLECT HANDLING FEE OPERATION START

$(document).on('click','.collect-handling-fee',function(){
    $('#confirm-delete').modal('show');
    $('#access-link').val($(this).data('href'));
});

// COLLECT HANDLING FEE OPERATION OPERATION END
...
if(admin_loader == 1)
 ...
 // CATALOG OPTION ENDS
    ...
    $('#confirm-kol').on('show.bs.modal', function(e) {
      console.log($(e.relatedTarget).data('href'));
       $(this).find('.btn-ok').attr('href', $(e.relatedTarget).data('href'));
    });

    $('#confirm-kol .btn-ok').on('click', function(e) {
      if(admin_loader == 1)
      {
          $('.submit-loader').show();
      }
    ...
    $('#confirm-special-kol').on('show.bs.modal', function(e) {
      console.log($(e.relatedTarget).data('href'));
       $(this).find('.btn-ok').attr('href', $(e.relatedTarget).data('href'));
    });

    $('#confirm-special-kol .btn-ok').on('click', function(e) {
      if(admin_loader == 1)
      {
          $('.submit-loader').show();
      }
      ...
      $.ajax({
          ...
          success:function(data)
          {
              $('#confirm-special-kol').modal('toggle');
              table.ajax.reload();
              $('.alert-danger').hide();
              $('.alert-success').show();
              $('.alert-success p').html(data);
      ...
      $('#confirm-transfer-point').on('show.bs.modal', function(e) {
          console.log($(e.relatedTarget).data('href'));
          $(this).find('.btn-ok').attr('href', $(e.relatedTarget).data('href'));
      });

      $('#confirm-transfer-point .btn-ok').on('click', function(e) {
          if(admin_loader == 1)
          {
              $('.submit-loader').show();
          }
          ...
          $.ajax({
              ...
              success:function(data)
              {
                  $('#confirm-transfer-point').modal('toggle');
                  table.ajax.reload();
                  $('.alert-danger').hide();
                  $('.alert-success').show();
                  $('.alert-success p').html(data);
  ...
  $('#confirm-verify .btn-ok').on('click', function(e) {
      if(admin_loader == 1)
      {
          $('.submit-loader').show();
      }
  ...
  $('#confirm-approve .btn-ok').on('click', function(e) {
    if(admin_loader == 1)
    {
      $('.submit-loader').show();
    }
  ...
  $('#confirm-update .btn-ok').on('click', function(e) {
    if(admin_loader == 1)
    {
      $('.submit-loader').show();
    }
...
// refresh-rank Start
$(document).on('click','.refresh-rank',function () {
  if(admin_loader == 1)
  {
      $('.submit-loader').show();
  }
  ...
  $.ajax({
    ...
    success:function(data)
    {
      if(data == 2){
        ...
        $.notify("User's rank is corrected.","success");
      }
      else{
...
$(document).on('click','#notf_order',function(){
  ...
  console.log($("#order-notf-show").data('href'));
});

$(document).on('click','#notf_order',function(){
	
	console.log($("#order-notf-wallet").data('href'));
    $('#order-notf-wallet').load($("#order-notf-wallet").data('href'));
  });
...
// RESEND-ORDER-INFO SECTION
$(document).on('click','.resend-order-info',function(){
  var url = $(this).data('href');
  var token = $('#emailreply').find('input[name=_token]').val();
  $('.close').click();
  $.notify("Attaching order info to customer !!","success");
  $.ajax({
    type: 'post',
    url: url,
    data: {
    ...
    success: function( data) {
      ...
      if(data == true){
		...
		$.notify("Oops Something Goes Wrong !! Send email failed!!","error");
	  }
    }
  });
...
// RESEND-ORDER-INFO SECTION ENDS

// RESEND-ORDER-INFO-VENDORS SECTION
$(document).on('click','.resend-order-info-vendors',function(){
  var url = $(this).data('href');
  var token = $('#emailreply').find('input[name=_token]').val();
  $('.close').click();
  $.notify("Attaching order info to vendors !!","success");
  $.ajax({ 
    type: 'post',
    url: url,
    data: {
...
$(document).on('click','.onepay-query-button', function(){
  $.ajax({
    type: 'get',
