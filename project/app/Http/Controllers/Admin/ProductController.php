...
class ProductController extends Controller
{
    ...
    //*** JSON Request
    public function datatables($category, $highlight, $from = null, $to = null, $key = null)
    {
         $datas = $this->query($category, $highlight, $from, $to, $key)
            ->orderBy('id','desc')
            ->limit(200)
            ->get();
    ...
    public function query($category, $highlight, $from = null, $to = null, $key = null)
    {
            ...
            $query = $query->where(function ($q) use ($key) {
                $q->where('name' , 'like', '%'.$key.'%')
                    ...
                    ->orWhere(function ($q) use ($key) {
                        $q = $q->whereExists(function ($q) use ($key) {
                            $q->select(DB::raw(1))
                                ->from('users')
    ...
    public function index()
    {
        ...
        return view('admin.product.index', compact('now'));
    }
