...
// DELETE OPERATION
      ...
      $('#confirm-delete .btn-ok').on('click', function(e) {
        if($('#confirm-delete .btn-ok').hasClass("order-btn")){
          console.log('#confirm-delete .btn-ok');
if(admin_loader == 1)
  {
  $('.submit-loader').show();
}
    ...
    $('#confirm-delete2 .btn-ok').on('click', function(e) {
      if($('#confirm-delete2 .btn-ok').hasClass("order-btn")){
        console.log('#confirm-delete2 .btn-ok');
        if(admin_loader == 1)
        {
          $('.submit-loader').show();
        }
      }
      var select_id = "#order-number-" + $(this).data('val');
      $.ajax({
        type:"GET",
        url:$(this).attr('href'),
        success:function(data)
        {
          ...
          $('.alert-success').show();
          $('.alert-success p').html(data[0]);
          $(select_id).attr('class','vendor-btn '+data[1]);
        }
      });
...
$(document).ready(function(){
    setInterval(function(){
        var url = $("#user-notf-count").data('href');
        if(url != undefined){
          $.ajax({
            type: "GET",
            url:url,
