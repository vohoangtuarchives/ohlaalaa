...
// -------------------------------- HOME PAGE SECTION ----------------------------------------
	...
	public function index(Request $request)
	{
        ...
        $selectable = ['id','user_id','name','slug','features','colors','photo','thumbnail','price','previous_price','attributes','size','size_price','discount_date','weight',
        ...
        $sliders = DB::table('sliders')->orderBy('ordered')->get();
        ...
        $feature_products =  Product::with('user')
          ...
          $discount_products =  Product::with('user')
            ->where('is_discount','=',1)
            ->where('status','=',1)
            ->where('discount_date', '>',  Carbon::today())
            ->inRandomOrder()
            ->limit(18)
            // ->orderBy('id','desc')
            // ->take(8)
...
// CURRENCY SECTION ENDS
    ...
    public function autoProvince($selected_city_id)
    {
        $provinces = Province::orderBy('name', 'asc')->get();
        return view('load.provinces',compact('provinces', 'selected_city_id'));
    }
...
{
    $this->code_image();
    $preferrers = User::where('preferred', PreferredType::Preferred)->orderBy('preferred_at','desc')->paginate(15);
        if($request->ajax()){
            return view('front.pagination.preferred',compact('preferrers'));
        }
    return view('front.preferred',compact('preferrers'));
}

// -------------------------------- PREFERRED SECTION ENDS----------------------------------------
...
// Maintenance Mode
    ...
    public function track_payment_status($id)
    {
        $order = Order::where('order_number','=',$id)->first();
