...
class GeneralSettingController extends Controller
{
    ...
    public function kol()
    {
        $data = KolConfig::where('kol_date', '=',  date('m-Y'))->first();
        if (empty( $data)) {
            $data = new \stdClass();
            $data->kol_date = date('m-Y');
        }
        
        return view('admin.generalsetting.kol',compact('data'));
    }
    ...
    public function getKol($date)
    {
        ...
        if ($data == null) {
            // $data = new \stdClass();
            // $data->kol_date = $date;
            return response()->json(['errors' => 'Please set config to time '.$date], 400);
        }

        return response()->json($data);
    }

    public function kolupdate(Request $request)
    {
        if (isset($request->kol_id) && $request->kol_id != null) {
            $kol = KolConfig::findOrFail($request->kol_id);

            if (is_null($kol)) {
                return response()->json('data not found');
            }

            $kol->con_bonus_l1 = $request->con_bonus_l1;
            $kol->number_orders_l1 = $request->number_orders_l1;
            $kol->number_users_l1 = $request->number_users_l1;
            $kol->number_shops_l1 = $request->number_shops_l1;
            $kol->avg_amount_order_l1 = $request->avg_amount_order_l1;
            $kol->number_affiliate_member_l1 =$request->number_affiliate_member_l1;
            $kol->con_bonus_l2 = $request->con_bonus_l2;
            $kol->number_orders_l2 = $request->number_orders_l2;
            $kol->number_users_l2 = $request->number_users_l2;
            $kol->number_shops_l2 = $request->number_shops_l2;
            $kol->avg_amount_order_l2 = $request->avg_amount_order_l2;
            $kol->number_affiliate_member_l2 =$request->number_affiliate_member_l2;
            $kol->kol_date = $request->kol_date;
            $kol->revenue_l1 = $request->revenue_l1;
            $kol->revenue_l2 = $request->revenue_l2;
            $kol->update();
        } else {
            $kol = new KolConfig;
            $kol->con_bonus_l1 = $request->con_bonus_l1;
