        ...
        // <a data-href="' . route('admin-prod-feature',$data->id) . '" class="feature" data-toggle="modal" data-target="#modal2"> <i class="fas fa-star"></i> Highlight</a>
         return Datatables::of($datas)
                            ...
                            ->editColumn('price', function(Product $data) {
                                ...
                                return  $sign->sign.number_format($price);
                            })
                            ->editColumn('price_shopping_point', function(Product $data) {
                                return  number_format($data->price_shopping_point);
                            })
                            ...
                            ->addColumn('action', function(Product $data) {
                                ...
                                $s = $data->status == 1 ? 'selected' : '';
                                $ns = $data->status == 0 ? 'selected' : '';
                                //$status = '<select class="process select droplinks '.$class.'"><option data-val="1" value="'. route('vendor-prod-status',['id1' => $data->id, 'id2' => 1]).'" '.$s.'>'.$this->vendor_language->lang713.'</option><option data-val="0" value="'. route('vendor-prod-status',['id1' => $data->id, 'id2' => 0]).'" '.$ns.'>'.$this->vendor_language->lang714.'</option></select>';
                                return '<div class="action-list"><a href="' . route('vendor-prod-edit',$data->id) . '"> <i class="fas fa-edit"></i>'.$this->vendor_language->lang715.'</a><a href="javascript" class="set-gallery" data-toggle="modal" data-target="#setgallery"><input type="hidden" value="'.$data->id.'"><i class="fas fa-eye"></i> '.$this->vendor_language->lang716.'</a><a data-href="' . route('vendor-prod-feature',$data->id) . '" class="feature" data-toggle="modal" data-target="#modal2"> <i class="fas fa-star"></i> Highlight</a><a href="javascript:;" data-href="' . route('vendor-prod-delete',$data->id) . '" data-toggle="modal" data-target="#confirm-delete" class="delete"><i class="fas fa-trash-alt"></i></a></div>';
                            })
...
//                $input['childcategory_id'] = $request->childcategory_id;
    ...
    public function store(Request $request)
    {
        ...
        if(!$package){
            return response()->json(array('errors' => [ 0 => 'Bạn chưa đăng ký mở tài khoản shop.']));
        }
        ...
        //--- Logic Section
            ...
            if(empty($request->whole_check ))
            {
                ...
                if(isset($request->whole_sell_qty)){
                    if(in_array(null, $request->whole_sell_qty))
                    {
                        $input['whole_sell_qty'] = null;
                    }
                    else
                    {
                        ...
                        if(isset($request->whole_sell_discount)){
                            if(in_array(null, $request->whole_sell_discount))
                            {
                                $input['whole_sell_discount'] = null;
                            }
                            else
            ...
            if($request->type == "License")
            {
                if(isset($request->license)){
                    if(in_array(null, $request->license))
                    {
                        $input['license'] = null;
                    ...
                    else
                    {
                        ...
                        if(isset($request->license_qty)){
                            if(in_array(null, $request->license_qty))
                            {
                                $input['license_qty'] = null;
                            }
                            else
        ...
        //--- Redirect Section
        $msg = 'Sản phẩm đã được thêm thành công. <a href="'.route('vendor-prod-index').'">Xem danh sách sản phẩm.</a>';
        return response()->json($msg);
        ...
        {
            //--- Redirect Section
            return response()->json(array('errors' => [ 0 => 'Không thể thêm sản phẩm được nữa.']));
    ...
    public function update(Request $request, $id)
    {
        ...
        {
            if(isset($request->license) && isset($request->license_qty)){
        ...
        if(isset($request->features) && isset($request->colors)){
            // Check Features
        ...
        if (file_exists($photoPath) && $prod->photo != '') {
            // Set Photo
            $resizedImage = Image::make(public_path().'/assets/images/products/'.$prod->photo)->resize(800, null, function ($c) {
                $c->aspectRatio();
            });
        ...
        //--- Redirect Section
        $msg = 'Sản phẩm đã được cập nhật. <a href="'.route('vendor-prod-index').'">Xem danh sách sản phẩm.</a>';
        return response()->json($msg);
        //--- Redirect Section Ends
    }
    ...
    public function catalogupdate(Request $request, $id)
    {
        ...
        //--- Redirect Section
        $msg = 'Sản phẩm được thêm thành công.<a href="'.route('vendor-prod-index').'"> Xem danh sách sản phẩm.</a>';
        return response()->json($msg);
        //--- Redirect Section Ends
        }
        else
        {
          //--- Redirect Section
          return response()->json(array('errors' => [ 0 => 'Không thêm sản phẩm được nữa.']));
